...
//        UartSendUartError(CmdDataError, SeqNumber);
        ...
        emberEventControlSetDelayMS(NwkFormEventControl,1500);
    }
    return UartCmdStatus;
}
...
int8u HC_GetNetworkInfo_Task(void) {
    ...
    EmberStatus = emberAfGetNetworkParameters(&nodeType, &networkParams);
    if(EmberStatus == EMBER_SUCCESS){
        if(networkParams.channels != 0){
            uint8_t Mac[8];
            emberAfGetEui64(Mac);
            SwapEndiannessEUI64(Mac);
            UartSendNwkInfo(Mac, networkParams.panId,
...
int8u HC_NetworkPjoin_Task(int8u *data) {
    ...
    if(PjoinTimes != 0x00){
        ...
        emberEventControlSetActive(BlinkLedWhenJoinEventControl);
    }
    else{
...
int8u HC_PjoinStatus_Task(void) {
    ...
    emberSerialPrintf(APP_SERIAL,"\n\r");
    #endif //Debug	
    UartSendPjoinStatus((int8u) emberGetPermitJoining());
    return UartCmdStatus;
...
int8u HC_ResetDevice_Task(int8u *data) {
    ...
    if(NwkAddr == 0x0000){
        ...
        emberEventControlSetDelayMS(DevResetEventControl,TimeDelay + 500);
    }
    else{
...
//        int8u seqNumber = CmdPayload[0];   
        ...
        int16u NwkAddr = MERGE16(CmdPayload[2],CmdPayload[1]);
        int8u status = emberActiveEndpointsRequest (NwkAddr,
                                    EMBER_APS_OPTION_RETRY | EMBER_APS_OPTION_ENABLE_ROUTE_DISCOVERY);
#ifdef Debug
...
//        int8u seqNumber = CmdPayload[0];  
        ...
        emberLqiTableRequest(ApsNwkAddr, 0, EMBER_AF_DEFAULT_APS_OPTIONS);
    }
...
int8u HC_GotoUpdateFirmware_Task(void) {
    int8u UartCmdStatus = UartCmdNormal;
    #ifdef Debug
    emberSerialPrintf(APP_SERIAL,"		Start Update Firmware... \n\r");
    emberSerialPrintf(APP_SERIAL,"\n\r");
    #endif //Debug
    UartSendConfirmUpdateFirmware();
    emberEventControlSetDelayMS(DeviceChangeToBootModeEventControl,500);
    return UartCmdStatus;
}
